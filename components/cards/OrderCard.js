...
function OrderCard({ orderObj }) {
  ...
  const fetchDetails = () => {
    if (orderObj.userId === loggedInUserId) {
      getUserById(orderObj.userId).then(setBuyer);
    }
  };
  ...
  return (
    orderObj.userId === loggedInUserId && (orderObj.isClosed || !orderObj.isClosed) ? (
      <Card style={{ width: '22rem', margin: '20px' }}>
        <Card.Body className="text-center">
          ...
          <Link href={`orders/${orderObj.id}`} passHref>
            <Button className="user-card-button" variant="warning">VIEW DETAILS</Button>
          </Link>
        </Card.Body>
      </Card>
    ) : null
